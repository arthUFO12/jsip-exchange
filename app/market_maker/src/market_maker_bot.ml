...
let initialize_config_from_context
  ...
  =
  List.iter config.symbol_configs ~f:(fun symbol_config ->
    symbol_config.fair_value_cents
    <- Context.fundamental context symbol_config.symbol |> Price.to_int_cents);
...
module T : Jsip_bot_runtime.Bot_runtime.Bot = struct
  ...
  let on_start (config : Config.t) (context : Bot_runtime.Context.t) =
    ...
    let symbol_list =
      Market_maker_data.get_symbol_list (get_mm_data config.mm_data)
    in
    Deferred.List.iter symbol_list ~how:`Parallel ~f:(fun symbol ->
      let cfg =
        Market_maker_data.get_config (get_mm_data config.mm_data) symbol
      in
      let sub order =
        Bot_runtime.Context.submit context order >>| Or_error.ok_exn
      in
      Market_maker.seed_book
        (get_mm_data config.mm_data)
        symbol
        cfg.fair_value_cents
        sub)
  ;;
  ...
  let on_event
    ...
    in
    let cancel coid =
      Bot_runtime.Context.cancel context coid >>| Or_error.ok_exn
    in
    return
      (Market_maker.repost_after_fill
         (get_mm_data config.mm_data)
         event
         submit
         cancel)
