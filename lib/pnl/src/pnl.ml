...
module TickerData = struct
  ...
  let update ?amount ?total_price ?trading_price t =
    ...
    let tot_price =
      match total_price with None -> t.total_price | Some price -> price
    in
    let trad_price =
      match trading_price with
      | None -> t.trading_price
      | Some price -> price
    in
    t.amount <- amt;
  ...
  let multiply_size_by_price size price =
    Int.to_float (Size.to_int size) *. Price.to_float price
    |> Price.of_float_exn
  ;;

  let buy_ticker t ~size ~price =
    let total_order_price = multiply_size_by_price size price in
    update
      t
      ~amount:(Size.( + ) t.amount size)
      ~total_price:(Price.( + ) t.total_price total_order_price)
  ;;
...
module ParticipantPnl = struct
  ...
  let _calculate_unrealized_pnl t =
    let fold_func ~key:_ ~data:t_data sum =
      ...
      | amt when Size.to_int amt >= 0 ->
        Price.to_float t_data.trading_price
        -. TickerData.average_entry_price t_data
        +. sum
      | _ ->
        TickerData.average_entry_price t_data
        -. Price.to_float t_data.trading_price
        +. sum
    in
    t.unrealized_pnl <- Price.of_float_exn (Hashtbl.fold t.ticker_data ~init:0.0 ~f:fold_func)
  ;;

  let exit_position t t_data ~side ~price ~(size : Size.t) =
    ...
    if (Side.equal side Buy && Size.( < ) amount Size.zero)
       || (Side.equal side Sell && Size.( > ) amount Size.zero)
    then (
      ...
      let transaction_pnl =
        ...
        | Buy -> tot_exit_price -. tot_entry_price
        | Sell -> tot_entry_price -. tot_exit_price
      in
      t.realized_pnl
      <- Price.of_float_exn transaction_pnl |> Price.( + ) t.realized_pnl)
  ;;

  let trade_ticker t ~side ~symbol ~price ~size =
    let t_data =
      Hashtbl.find_or_add t.ticker_data symbol ~default:TickerData.create
    in
...
type t =
  { participant_data :
      (Participant.t, ParticipantPnl.t) Hashtbl_intf.Hashtbl.t
  }

let _create () = { participant_data = Hashtbl.create (module Participant) }

let _apply_fill t (fill : Fill.t) =
  let aggressor_data =
    Hashtbl.find_or_add
      t.participant_data
      fill.aggressor_participant
      ~default:(fun () -> ParticipantPnl.create fill.aggressor_participant)
  in
  let resting_data =
    Hashtbl.find_or_add
      t.participant_data
      fill.resting_participant
      ~default:(fun () -> ParticipantPnl.create fill.resting_participant)
  in
  ParticipantPnl.trade_ticker
    aggressor_data
