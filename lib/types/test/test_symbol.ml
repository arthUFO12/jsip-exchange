...
let%expect_test "of_string: empty string raises" =
  ...
  [%expect {| "Symbol.of_string: symbol must be a non-empty alphanumeric string" |}]
;;
...
let%expect_test "of_string: uncapitalized alphanumeric string passes and is capitalized" =
  [%test_result: string]
  (let aapl = "aapl" in
  [%string "%{Symbol.of_string aapl#Symbol}"]
  )
