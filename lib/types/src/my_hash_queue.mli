(** A FIFO queue with O(1) removal by key.
    ...
    constant-time lookup). Unlike a plain queue, an element can be pulled
    out of the middle in O(1) given its key — see {!remove}. Keys are
    unique: {!enqueue} rejects a key that is already present.
...
(** [create m] is an empty queue keyed by the module [m], e.g.
    [create (module String)]. [m] supplies hashing and equality for the
    keys. *)
...
(** [dequeue t] removes and returns the value at the front of [t] (the
    least recently enqueued), or [None] if [t] is empty. *)
...
val sexp_of_t : ('k -> Sexp.t) -> ('v -> Sexp.t) -> ('k, 'v) t -> Sexp_type.Sexp.t