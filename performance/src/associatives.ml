...
module Map_int = struct
  ...
  let create () : t = ref Int.Map.empty
  let set (t : t) ~key ~data = t := Map.set !t ~key ~data
  let get (t : t) key = Map.find !t key
end
...
module Hashtable_int = struct
  ...
  let get (t : t) key = Hashtbl.find t key
end

module Map_string = struct
  type t = int String.Map.t ref

  let create () : t = ref String.Map.empty
  let set (t : t) ~key ~data = t := Map.set !t ~key ~data
  let get (t : t) key = Map.find !t key
end
...
module Map_record = struct
  ...
  let create () : t = ref Fat_record.Map.empty
  let set (t : t) ~key ~data = t := Map.set !t ~key ~data
  let get (t : t) key = Map.find !t key
end
